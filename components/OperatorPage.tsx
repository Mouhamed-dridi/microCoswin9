  ...
  useEffect(() => {
    const fetchMachines = async () => {
      try {
        ...
        const flatMachines = data.flat().filter((m: any) => m && m.id && m.nameOrCode);
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <select required className="w-full h-11 px-3.5 bg-white border border-[#D0D5DD] rounded-lg text-sm" value={formData.machineCode} onChange={e => setFormData({ ...formData, machineCode: e.target.value })}>
                <option value="" disabled>Sélectionner Machine</option>
                {machines.map((m: any) => (
                  <option key={m.id} value={m.nameOrCode}>{m.nameOrCode}</option>
                ))}
              </select>
            </div>
            <div className="space-y-1.5">
