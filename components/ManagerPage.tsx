...
type TabType = 'settings' | 'groups-users' | 'crm' | 'tickets' | 'documentation' | 'rapport' | 'maintenance-request';
    ...
    <div className="flex h-screen w-full bg-[#FCFCFD] font-['Plus_Jakarta_Sans'] text-[#101828]">
      <aside className="w-[280px] bg-[#F9FAFB] border-r border-[#EAECF0] flex flex-col no-print shrink-0">
        ...
        <nav className="flex-1 px-4 space-y-1 overflow-y-auto">
          ...
          <SidebarItem id="maintenance-request" label="Maintenance Request" active={activeTab === 'maintenance-request'} icon={<svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><circle cx="12" cy="12" r="3" /></svg>} />
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-white overflow-hidden">
        ...
        <main className="flex-1 overflow-hidden flex flex-col p-8 w-full">
          ...
          ) : activeTab === 'crm' ? <CRMPage /> : activeTab === 'settings' ? <SettingsPage /> : activeTab === 'groups-users' ? <GroupesUtilisateursPage /> : activeTab === 'maintenance-request' ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center p-12">
              <div className="w-16 h-16 bg-[#F2F4F7] rounded-full flex items-center justify-center mb-4">
                <svg className="w-8 h-8 text-[#667085]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><circle cx="12" cy="12" r="3" /></svg>
              </div>
              <h2 className="text-xl font-bold text-[#101828] mb-2">Maintenance Request</h2>
              <p className="text-[#667085] max-w-sm">Gérez et suivez toutes les demandes de maintenance industrielle ici.</p>
            </div>
          ) : <div>Placeholder</div>}
        </main>
      </div>
